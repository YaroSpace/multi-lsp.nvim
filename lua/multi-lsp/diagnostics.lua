...
local function handle_diagnostics(uri, client_id, diagnostics, is_pull, config)
  ...
  -- if bufnr == 4 then
  -- LOG(debug.getinfo(3).source, debug.getinfo(3).linedefined, debug.getinfo(4), debug.getinfo(4).linedefined)
  --   if diagnostics.user_data then LOG(namespace, diagnostics.user_data.lsp.data.code_actions[1].kind) end
  -- end
